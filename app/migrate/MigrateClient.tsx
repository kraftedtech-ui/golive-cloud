...
const PRICING_DISPLAY: Record<string, { best: string; color: string; popular?: boolean }> = {
  starter: { best: 'Small teams 1–20 users', color: CY },
  secure: { best: 'Businesses handling sensitive data', color: NAVY, popular: true },
  ai: { best: 'Mid-size teams ready for Copilot AI', color: TEAL },
}
...
export default function MigrateClient({ pricing }: { pricing: PublicPackage[] }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // if (!emailVerified || !verificationToken) { setStatus('error'); return }
    if (!turnstileToken) {
      setStatus('error')
      return
    }
    ...
    <div style={{ fontFamily: 'Segoe UI,system-ui,sans-serif', background: '#f4fafd', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <p style={{ textAlign: 'center', color: MUTED, fontSize: 14, marginBottom: 24 }}>Setup and migration included in all packages. Prices shown are for annual commitment — flexible monthly billing also available.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {pricing.map(pkg => {
              const display = PRICING_DISPLAY[pkg.key] || { best: '', color: CY }
              const priceLabel = pkg.priceUsd === null ? 'Custom' : `$${pkg.priceUsd}`
              return (
                <div key={pkg.key} style={{ background: display.popular ? NAVY : '#fff', border: `2px solid ${display.popular ? NAVY : BORDER}`, borderRadius: 12, padding: '24px', position: 'relative' }}>
                  {display.popular && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: TEAL, color: NAVY, fontSize: 10, fontWeight: 700, padding: '3px 14px', borderRadius: 20 }}>MOST POPULAR</div>}
                  <div style={{ fontSize: 14, fontWeight: 700, color: display.popular ? 'rgba(255,255,255,.7)' : MUTED, marginBottom: 6 }}>{pkg.name}</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 4 }}>
                    <span style={{ fontSize: 36, fontWeight: 800, color: display.popular ? '#fff' : display.color }}>{priceLabel}</span>
                    {pkg.priceUsd !== null && <span style={{ fontSize: 13, color: display.popular ? 'rgba(255,255,255,.6)' : MUTED }}>/user/month</span>}
                  </div>
                  <div style={{ fontSize: 11, color: display.popular ? 'rgba(255,255,255,.5)' : MUTED, marginBottom: 16 }}>Best for: {display.best}</div>
                  {pkg.features.map(f => (
                    <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8, fontSize: 12, color: display.popular ? 'rgba(255,255,255,.8)' : '#333' }}>
                  ...
                  <button onClick={() => document.getElementById('transfer-form')?.scrollIntoView({ behavior: 'smooth' })}
                    style={{ width: '100%', marginTop: 16, background: display.popular ? CY : 'transparent', color: display.popular ? '#fff' : CY, border: `2px solid ${CY}`, borderRadius: 8, padding: '10px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                    Get started →
                  </button>
                </div>
              )
          ...
          <p style={{ textAlign: 'center', color: MUTED, fontSize: 12, marginTop: 12 }}>
            * One-time setup fee applies: {pricing.map(p => p.setupFeeUsd !== null ? `$${p.setupFeeUsd} (${p.name})` : `Custom (${p.name})`).join(' · ')}. Includes full migration.
          </p>
        </div>
              ...
              <form onSubmit={submit}>
                ...
                <div style={{ marginBottom: 10 }}>
                  <div>
                    ...
                    {/* EMAIL OTP SUSPENDED — plain input only. To reactivate: restore disabled,
                        background/border logic, Verify button, emailWidgetRef div, Verified badge,
                        OTP input block, and otpError display. */}
                    <div style={{ display: 'flex', gap: 6 }}>
                      <input style={inp} required type="email" placeholder="you@company.com"
