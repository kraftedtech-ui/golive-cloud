...
const PACKAGES = [
  { name: "Starter", priceUsd: 6, tagline: "Professional email & productivity to get online fast.", features: ["Microsoft 365 Business Basic","Custom domain business email","1 TB OneDrive per user","Teams, Word, Excel & PowerPoint (web)","Local currency billing","Email & chat support"] },
  { name: "Secure Business", priceUsd: 22, tagline: "Productivity plus advanced security & device management.", featured: true, features: ["Microsoft 365 Business Premium","Microsoft Defender for Business","Desktop Office apps + 1 TB storage","Intune device management & MFA","Data loss prevention & encryption","Priority support + onboarding"] },
  { name: "AI Enterprise", priceUsd: null as number | null, priceLabel: "Custom", tagline: "Copilot, Azure & tailored enterprise security at scale.", features: ["Microsoft 365 Copilot licensing","Azure cloud & infrastructure","Defender for Office, Endpoint & Cloud","Power Platform automation","Dedicated account manager","Architecture & compliance review"] },
]

function PriceDisplay({ pkg }: { pkg: typeof PACKAGES[0] }) {
  const { format } = useCurrency()
  if (pkg.priceUsd === null) return <span className="text-4xl font-bold text-[#0d2233]">{pkg.priceLabel}</span>
  return (
    ...
    <section id="packages" className="relative bg-[#f4fafd]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:items-start">
          {PACKAGES.map((pkg) => (
            <div key={pkg.name} className={pkg.featured
              ...
              <p className="mt-1.5 min-h-10 text-sm leading-relaxed text-[#5a7a8a]">{pkg.tagline}</p>
